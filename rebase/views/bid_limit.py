...
class BidLimitSchema(RebaseSchema):
    ...
    ticket_snapshot = SecureNestedField('TicketSnapshotSchema', only=('id','title', 'ticket'), required=True)
    ticket_set =      SecureNestedField('TicketSetSchema', only=('id','auction'), required=True)
