...
class TestCodeClearanceResource(AlveareRestTestCase):
    ...
    def test_get_one(self):
        ...
        self.assertTrue(code_clearance['contractor_id'])
        self.assertTrue(code_clearance['project_id'])
    ...
    def test_delete_project(self):
        code_clearance = self.r.get_any()
        self.delete_resource('projects/{}'.format(code_clearance['project_id']))
        self.get_resource(self.r.url(code_clearance), 404)

    def test_delete_contractor(self):
        code_clearance = self.r.get_any()
        self.delete_resource('contractors/{}'.format(code_clearance['contractor_id']))
        self.get_resource(self.r.url(code_clearance), 404)
